...
<html lang="en">
	...
	<body>
		...
		<div style="margin-top: 200px; margin-bottom: 170px; ">
			<h1 align="left" class="h1">Delete News</h1>
            ...
            <?php
                ...
                $sql = "SELECT news_id, heading FROM news ORDER BY news_id";
                $result = $conn->query($sql);

                if ($result->num_rows>0)
                {
                    echo "<table>";
                    echo "<tr><td><h4> Current News <h3></td></tr>";
                    while($row = $result->fetch_assoc())
                    {
                        echo "<tr>";

                        echo "<td> ID:".$row["news_id"]."</td><td>".$row["heading"]."</td>";
            ...
            <form action="" method="GET">
				<br>
                <label for="ID">Enter the News ID to delete<label><br>
				<input type="textbox" id="news_id" name="news_id">
				<br><br>
			...
			<?php
				if(isset($_GET['submit']))
				{
					...
					if ($conn->query($sql) === TRUE) 
                    {
						...
						echo '<script language="javascript">';
                        echo 'window.location.href = "../php/deletenews.php"';
                        echo '</script>';
                    } 
                    else 
                    {
						echo '<script language="javascript">';
                        echo 'alert("Wrong News ID")';
                        echo '</script>';
